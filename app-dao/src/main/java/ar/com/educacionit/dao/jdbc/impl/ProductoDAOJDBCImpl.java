...
public class ProductoDAOJDBCImpl implements ProductoDAO {
	...
	public Producto getProducto(String codigo) {
		...
		try {
			...
			if (res.next()) {
				producto = new Producto(res.getLong("ID"), res.getString("DESCRIPCION"), res.getDouble("PRECIO"),
	...
	public List<Producto> findProductos() throws GenericException {
		...
		try {
			...
			while (res.next()) {
				Producto p = new Producto(res.getLong("ID"), res.getString("DESCRIPCION"), res.getDouble("PRECIO"),
						res.getString("CODIGO"));
				productos.add(p);
			}
		} catch (Exception e) {
